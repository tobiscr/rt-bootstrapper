...
|FIPS mode enablement|The webhook sets an environment to the Pod which indicates to run in FIPS mode.|Add environment variable `KYMA_FIPS_MODE_ENABLED`.|  Append key-value array `.spec.containers[*].env[]` with `KYMA_FIPS_MODE_ENABLED=true`|`rt-cfg.kyma-project.io/set-fips-mode: "true"`|
